...
<?
		while($row = mysqli_fetch_array($cart_result)){
			...
			$color_name=$cart->get_product_color_name($row['product_color']);
?>
            ...
            <td height="60" colspan="4">
                <table border="1" cellpadding="6" class="list_item">
                  <tr>
                    ...
                    <td width="14%"><p><?=$row['item_count']?></p></td>
                    <td width="13%"><p id="subtotal_<?=$row['product_id']?>"><?=$row['item_price']*$row['item_count']?></p></td>
                  </tr>
                  <tr>
...
<?
		...
		$pay_price=$cart->get_buyer_discount();
?>
          <tr id="sum_column">
            <td height="60" colspan="4">
   	          <div class="payment_total_col">消費總金額：NT $<span class="price_total_style"><?=$pay_price?></span>元</div>
   	          <div class="payment_total_col">折扣金額：NT $<span class="price_total_style"><?=$sum_price-$pay_price?></span>元</div>
   	          <div class="payment_total_col">合計金額：NT $<span class="price_normal_style"><?=$sum_price?></span>元</div>
		...
		<h3>收件人資料</h3>
          <div class="buyer_data_left">
            <p class="send_col"><span class="send_data"><?=$_SESSION['buyer']['name']?></span><span class="send_title">姓名：</span></p>
            <p class="send_col"><span class="send_data"><?=$_SESSION['buyer']['tel']?></span><span class="send_title">電話：</span></p>
            <p class="send_col"><span class="send_data"><?=$_SESSION['buyer']['email']?></span><span class="send_title">E-MAIL：</span></p>
          </div>
          <div class="buyer_data_right">
            <p class="send_col"><span class="send_data"><?=$_SESSION['buyer']['zip_code']?></span><span class="send_title">郵遞區號：</span></p>
            <p class="send_col"><span class="send_data"><?=$_SESSION['buyer']['zip_county'].$_SESSION['buyer']['zip_area'].$_SESSION['buyer']['add']?></span><span class="send_title">住址：</span></p>
          </div>
<?
//	有效訂單
if(!$cancel_order){
...
?>
          <h3><?=$payment_title?></h3>
			<table class="table_payment_method">
              ...
              <tr>
                <td>&nbsp;<input type="hidden" name="action" value="buyer_to_pay" />
                					<input type="hidden" name="buyer_token" value="<?=$_SESSION['buyer_token']?>">
                </td>
